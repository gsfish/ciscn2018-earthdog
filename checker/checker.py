...
class WebChecker:
    def __init__(self, ip, port, csrfname='csrfmiddlewaretoken'):
        ...
        self.username = self._generate_randstr()
    ...
    def _get_id_price(self, html):
        dom = PQ(html)
        form = dom("form")
        pay_id = str(PQ(form[0])("input[name=\"id\"]").attr("value")).strip()
        pay_price = str(PQ(form[0])("input[name=\"price\"]").attr("value")).strip()
    ...
    def pay_test(self):
        integral = self._get_user_integral()
        rs = self.session.get(self.url + 'info?id=%d' % random.randint(1, 100))
        token = self._get_token(rs.text)
    ...
    def _get_amount(self, id):
        res = self.session.get(self.url + 'info/?id=%s' % str(id))
        dom = PQ(res.text)
    ...
    def second_kill_test(self):
        kill_id = random.randint(1, 100)
        amount = self._get_amount(kill_id)
        ...
        new_amount = self._get_amount(kill_id)

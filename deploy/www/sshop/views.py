...
def user(request):
    ...
    own = Owner.objects.filter(user=user)
    return render(request, 'user.html', {'user': user, 'is_admin': is_admin, 'owns': own})
...
def info_cid(request, cid):
    try:
        commodity = Commodity.objects.get(pk=cid)
    except Commodity.DoesNotExist:
        return HttpResponse('commodity not found (￣_￣ )', status_code=400)
    raw_user = True if not request.session.get('uid') else False
...
def pay(request):
    ...
    if shopcar:
        cid = next(serializers.deserialize('json', shopcar)).object.pk
        if cid != int(request.POST['id']):
            return HttpResponse('this is useless (=￣ω￣=)', status=400)
        cur_amount = Commodity.objects.get(pk=cid).amount
        cur_amount -= 1
        commodity = Commodity.objects.get(pk=cid)
        ...
        if user.username != 'admin' and cur_amount < 990:
        ...
        if cur_amount >= 0 and cur_integral >= 0:
            ...
            own.amount += 1
            own.save()
            return render(request, 'shopcar.html', {'commodity': commodity, 'success': 1})
...
def seckill(request):
    if request.method == 'POST':
        ...
        user = User.objects.get(pk=request.session['uid'])
        if user.integral - commodity.price < 0 or (user.username != 'admin' and commodity.amount < 990) or commodity.amount <= 0:
            return render(request, 'seckill.html', {'danger': 1})
        commodity.amount -= 1
        user.integral -= commodity.price
        user.save()
        commodity.save()
...
def shopcar_add(request):
    if not request.POST.get('id'):
        return HttpResponse('parameters invaild (￣_￣ )', status_code=400)
    try:
